...
Here is how our project is organized now that we've added documentation layers:

```text
📁 introduction-to-nest
├── 📄 DOCUMENTATION_TYPES.md   <-- Extra Lesson: Documentation Ecosystem
├── 📄 README.md                <-- Today's main guide
├── 📁 src
│   ├── 📄 main.ts              <-- Swagger initialization
│   ├── 📄 app.controller.ts    <-- Documented root endpoints
│   └── 📁 books
│       ├── 📄 books.controller.ts <-- Documented CRUD endpoints
│       └── 📁 dto
│           └── 📄 create-book.dto.ts <-- Documented Data Models (DTOs)
└── 📁 test                     <-- E2E tests
```
...
```mermaid
graph TD
    Code["💻 NestJS Code (Controllers/DTOs)"]
    Decorators["🏷️ Swagger Decorators (@ApiTags, @ApiProperty)"]
    Spec["📄 OpenAPI JSON Specification"]
    UI["🌐 Swagger UI (http://localhost:3000/api)"]

    Code --> Decorators
    Decorators -- "Generated by NestJS" --> Spec
    Spec -- "Rendered by" --> UI
```
...
flowchart LR
    ...
    SwaggerUI["🌐 Swagger UI Page"]
    Postman["📬 Postman Collection"]

    Nest -- "Generates" --> OpenAPI
    OpenAPI -- "Displayed in" --> SwaggerUI
    OpenAPI -- "Imported into" --> Postman
```
...
2. **Day 2:** Ensuring bad data doesn't crash our app using **ValidationPipes** and **DTOs**.
3. **Day 3:** Organizing our factory using **Layered Architecture**.
4. **Day 4:** Being a good teammate by providing clear **API Documentation**.
5. **Day 5:** Mastering **DTOs & Pipes** (Validation, Transformation, and Mapped Types).

---

## 📅 Day 5: Understanding DTO & Pipes in NestJS

Today we deep dive into how data flows safely into our system. We use **Pipes** for validation and transformation, and **Mapped Types** to keep our code DRY (Don't Repeat Yourself).
...
### 🛡️ Layer 1: Validation Pipe Options

In `main.ts`, we configure global protection rules.

```typescript
app.useGlobalPipes(new ValidationPipe({
  whitelist: true,               // Strip non-decorated properties
  forbidNonWhitelisted: true,    // Error if extra fields are sent
  transform: true                // Auto-transform payloads to DTO classes
}));
```

### 🔄 Layer 2: Transformation Pipes

We use built-in pipes like `ParseIntPipe` or `ParseUUIDPipe` to ensure our IDs are of the correct type before they reach our service.

```mermaid
flowchart LR
    URL["GET /products/:id (String)"]
    Pipe["🛠️ ParseIntPipe"]
...
### 🕶️ Layer 3: Mapped Types (Partial & Pick & Omit)

Mapped types allow us to reuse our DTOs for different scenarios (Updating, Creating, or Masking).

| Type           | Purpose                                      | Example                                     |
| -------------- | -------------------------------------------- | ------------------------------------------- |
| `PartialType`  | Makes all fields optional for Update         | `class UpdateDto extends PartialType(CDto)` |
| `OmitType`     | Removes specific fields (like `password`)    | `class SafeDto extends OmitType(UDto, ['password'])` |
| `PickType`     | Selects only a few fields                    | `class MiniDto extends PickType(PDto, ['name'])` |

---

> [!IMPORTANT]
> **Extra Lesson:** Don't forget to check out [Security Best Practices](./SECURITY_BEST_PRACTICES.md) to learn how to tie all these concepts together to build a bulletproof API!

## Concept Glossary

| Term               | Definition                                      | Usage                                           |
| ------------------ | ----------------------------------------------- | ----------------------------------------------- |
| `ValidationPipe`   | A filter that validates incoming data           | Configured globally in `main.ts`                |
| `Transformation`   | Converting data from one type to another        | Using `transform: true` or `ParseIntPipe`       |
| `PartialType`      | Utility to create an optional version of a DTO  | Used for `PATCH` requests                       |
| `OmitType`         | Utility to remove fields from a DTO             | Used to mask sensitive data like `password`     |
| `Repository`       | Pattern to separate data logic from business logic | `ProductsRepository` handles the mock data array |
